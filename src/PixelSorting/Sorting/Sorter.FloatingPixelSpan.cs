...
#pragma warning disable CS8500 // This takes the address of, gets the size of, or declares a pointer to a managed type
        ...
        where TPixel : struct
    {
        ...
        /// </summary>
        /// <typeparam name="TPixel"></typeparam>
        public readonly ref struct VerticalFloatingPixelSpan
        {
            /// <summary>A byref or a native ptr.</summary>
            internal readonly ref TPixel _reference;
            /// <summary>The number of elements this Span operates on.</summary>
            ...
            public VerticalFloatingPixelSpan(TPixel[] reference, double step, int lo, int hi)
            {
                _reference = ref reference[lo];
                int size = hi - lo;
                _items = (int)(size / step + (size % step == 0 ? 0 : 1));
            ...
            public VerticalFloatingPixelSpan(Span<TPixel> reference, double step, int lo, int hi)
            {
                _reference = ref reference[lo];
                int size = hi - lo;
                _items = (int)(size / step + (size % step == 0 ? 0 : 1));
            ...
            public VerticalFloatingPixelSpan(void* pointer, double step, int lo, int hi)
            {
                _reference = ref *((TPixel*)pointer + lo);
                int size = hi - lo;
                _items = (int)(size / step + (size % step == 0 ? 0 : 1));

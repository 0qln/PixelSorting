...
namespace Sorting
{
        ...
        where TPixel : struct
    {
        ...
        public static void IntrospectiveSort(VerticalFloatingPixelSpan keys, IComparer<TPixel> comparer)
        {
            if (keys.ItemCount <= 1) return;
            IntroSort(keys, comparer, 0, keys.ItemCount - 1, 2 * FloorLog2(keys.ItemCount));
        }
        ...
        // this is currenly not profitable.
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static void IntroSort(VerticalFloatingPixelSpan keys, IComparer<TPixel> comparer, int lo, int hi, int depthLimit)
        {
            while (hi > lo)
            {
                int partitionSize = hi - lo + 1;
        ...
        private static int PickPivotAndPartition(VerticalFloatingPixelSpan keys, IComparer<TPixel> comparer, int lo, int hi)
        {
            // Compute median-of-three.  But also partition them, since we've done the comparison.
            int middle = lo + ((hi - lo) / 2);
        ...
        private static void Swap(VerticalFloatingPixelSpan keys, int i, int j)
        {
            TPixel t = keys[i];
            keys[i] = keys[j];
            keys[j] = t;
        ...
        private static void SwapIfGreater(VerticalFloatingPixelSpan keys, IComparer<TPixel> comparer, int i, int j)
        {
            if (i != j)
            {
                if (comparer.Compare(keys[i], keys[j]) > 0)

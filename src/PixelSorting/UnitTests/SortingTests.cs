...
public class ComparisonTests
{
    ...
    {
        for (var a = 0xF; a != 0x0F000000; a <<= 0x1)
        {
            for (var b = 0xF; b != 0x0F000000; b <<= 0x1)
            {
                var result = new PixelComparer.Ascending.Red().Compare(a, b);
                var expected = new PixelComparer.Ascending.Red().Compare(ToUnionInt(a), ToUnionInt(b));
...
public class SortingTests
{
    ...
    [InlineData(100, 2, 0, 70)]
    [InlineData(100, 3, 30, 70)]
    public void CombSort_PixelSpan(int size, int step, int from, int to)
    {
        var comparer = new PixelComparer.Ascending.Red();
        ...
        foreach (var test in tests)
        {
            Sorter<int>.CombSort(new Sorter<int>.PixelSpan(test.Unsorted, test.Properties.Step, test.Properties.From, test.Properties.To), comparer);
            Assert.True(test.Sorted.SequenceEqual(test.Unsorted, EqualityComparer<int>.Create((a, b) => comparer.Compare(a, b) == 0)));
        }
    }
